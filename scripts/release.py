...
def get_pr_num_from_remote_branch(branch_name):
    # branch name should look like remotes/origin/pr/1234
    parts = branch_name.split("/")
    if len(parts) != 4:
        raise InvalidBranchName(branch_name)
    if parts[0] != "remotes" or parts[1] != "origin" or parts[2] != "pr":
        raise InvalidBranchName(branch_name)
    try:
        num = int(parts[3])
    except ValueError:
...
def prs_by_email(commit_range):
    ...
    for commit in Commit.iter_items(repo, commit_range):
        ...
        try:
            pr_num = get_pr_num_from_remote_branch(remote_branch)
        except InvalidBranchName:
            print(
                "Invalid remote branch {branch} for commit {commit}".format(
                    branch=remote_branch, commit=commit,
                ),
                file=sys.stderr,
            )
        merge = get_merge_commit(commit)
        unordered_data[merge.author.email].add((pr_num, merge.authored_date))

    ordered_data = OrderedDict()
    for email in sorted(unordered_data.keys()):
        ordered = sorted(unordered_data[email], key=lambda num_date: num_date[1])
        ordered_data[email] = [num for num, date in ordered]
    return ordered_data
